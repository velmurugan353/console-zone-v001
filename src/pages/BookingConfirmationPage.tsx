...
export default function BookingConfirmationPage() {
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      ...
      try {
        const docRef = doc(db, 'rentals', bookingId);
  ...
  const bookingRef = bookingId ? `ID: ${bookingId.slice(0, 8)}...` : 'N/A';
        ...
        <div className="bg-[#0a0f1e] border border-white/10 rounded-3xl p-8 md:p-12 text-left space-y-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row gap-8 relative z-10">
            <div className="w-32 h-32 rounded-2xl overflow-hidden bg-black/40 border border-white/5 shrink-0">
              <img src={consoleData.image} alt={consoleData.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            </div>
            <div className="flex-1 space-y-4">
              <div>
                <h4 className="text-2xl font-black uppercase tracking-tight text-white">{consoleData.name}</h4>
