...
export default function AdminOrders() {
  ...
  const handleEditToggle = async () => {
    if (isEditing) {
      if (selectedOrder) {
        const updatedOrder = { ...selectedOrder, ...editForm } as Order;
        try {
          await updateDoc(doc(db, 'orders', updatedOrder.id), editForm);
  ...
  const handleStatusChange = async (id: string, newStatus: OrderStatus, trackingNumber?: string) => {
    ...
    try {
      await updateDoc(doc(db, 'orders', id), updatedOrderData);

      if (selectedOrder?.id === id) {
        setSelectedOrder({ ...order, ...updatedOrderData });
      }
    } catch (error) {
